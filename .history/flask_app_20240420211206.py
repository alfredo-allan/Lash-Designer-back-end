...
@app.route('/dados', methods=['GET']) def get_dados(): dados = c.execute("SELECT * FROM dados").fetchall()
...
# Alterando a ordem das chaves no dicionário
for item in formatted_data:
    item["dataVencimento"], item["descricaoProduto"], item["ean"], item["enderecoCliente"], item["nomeCliente"], item["nomePromotor"], item["quantidadeProdutos"] = item.pop("dataVencimento"), item.pop("descricaoProduto"), item.pop("ean"), item.pop("enderecoCliente"), item.pop("nomeCliente"), item.pop("nomePromotor"), item.pop("quantidadeProdutos")
