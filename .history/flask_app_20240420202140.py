...
def get_dados():
    dados = c.execute("SELECT nomePromotor, nomeCliente, enderecoCliente, ean, descricaoProduto, dataVencimento, quantidadeProdutos FROM dados").fetchall()
    response_data = []
    for dado in dados:
        response_data.append({
            "nomePromotor": dado[1],
            "nomeCliente": dado[2],
            "enderecoCliente": dado[3],
            "ean": dado[4],
            "descricaoProduto": dado[5],
            "dataVencimento": dado[6],
            "quantidadeProdutos": dado[7]
        })
