...
Neste código, foi adicionada a geração do token utilizando a biblioteca JWT (JSON Web Tokens) e uma chave secreta. Além disso, a função `require_token` foi atualizada para verificar se o token está presente, se é válido e se o usuário correspondente é o admin. Isso garante que apenas o admin tenha acesso aos dados protegidos.