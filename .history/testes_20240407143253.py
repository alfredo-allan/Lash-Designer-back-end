...
def test_save_dados(client): novo_dado = { "nome": "Teste", "whatsapp": "123456789", "dia": "01/01/2022", "horario": "08:00", "tipo_cilio": "Teste" } response = client.post('/dados', json=novo_dado) data = response.get_json() assert response.status_code == 201 assert data is not None # Verifica se os dados foram salvos corretamente no servidor assert data['nome'] == "Teste" assert data['whatsapp'] == "123456789" assert data['dia'] == "01/01/2022" assert data['horario'] == "08:00" assert data['tipo_cilio'] == "Teste"
